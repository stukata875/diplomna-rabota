<?php
include 'config.php';
$page_title = 'Литература за тийнеджъри - Младежки романи';
include 'header.php';
...
// Вземане на общия брой книги за тийнеджъри
$total_stmt = $conn->prepare("SELECT COUNT(*) as total FROM books WHERE is_teen = 1");
$total_stmt->execute();
...
<style>
/* Стилове за Литература за тийнеджъри */
.page-content-teen {
    max-width: 1400px;
    margin: 0 auto;
    padding: 20px;
...
/* Стилове за действия с книгата - Тийнеджъри */
.book-actions-teen {
    display: flex;
    gap: 10px;
    margin-top: auto;
...
.book-actions-teen .add-to-cart-btn-teen {
    flex: 1;
    margin-top: 0;
    padding: 12px 0;
}
...
.add-to-wishlist-btn-teen:hover {
    background: #ec407a;
    color: white;
    transform: translateY(-2px);
...
.add-to-wishlist-btn-teen.added {
    background: #ec407a;
    color: white;
    border-color: #ec407a;
}
...
.pagination-teen a:hover {
    background: #ec407a;
    color: white;
    border-color: #ec407a;
}

.pagination-teen a.active-page {
    background: #ec407a;
    color: white;
    border-color: #ec407a;
    font-weight: bold;
...
<div class="page-content-teen">
    <h1>Литература за тийнеджъри</h1>
    
    <?php if ($total_books > 0): ?>
    <div class="page-info-teen">
    ...
    <?php
    if ($total_books > 0) {
        // Вземане на книги за тийнеджъри
        $sql = "
            SELECT * FROM books 
            WHERE is_teen = 1 
    ...
    <div class="books-grid-teen">
        <?php 
        $counter = $offset + 1;
        while ($book = $result->fetch_assoc()): 
        ?>
        <div class="book-card-teen">
            <div class="book-image-teen">
                <img src="<?= htmlspecialchars($book['image']) ?>" 
                     alt="<?= htmlspecialchars($book['title']) ?>"
                     onerror="this.src='https://via.placeholder.com/120x160/fce4ec/ec407a?text=Тийнеджър'">
            </div>
            
            <div class="book-title-teen">
                <h3>
                    <a href="book.php?id=<?= $book['id'] ?>" 
                       class="book-title-link-teen">
                        <?= htmlspecialchars($book['title']) ?>
                    </a>
                </h3>
            </div>
            
            <div class="book-author-teen">
                <a href="author_books.php?author=<?= urlencode($book['author']) ?>" 
                   class="author-link-teen">
                    <?= htmlspecialchars($book['author']) ?>
                </a>
            </div>
          <div class="price-sales-teen">
    <span class="book-price-teen"><?= number_format($book['price'], 2) ?> лв.</span>
    <span class="book-sales-teen"><?= $book['sales'] ?> продажби</span>
</div>

<!-- Бутони за действия с книгата -->
<div class="book-actions-teen">
    <button class="add-to-cart-btn-teen" 
            onclick="addToCartTeen(<?= $book['id'] ?>, this)">
        Добави в кошницата
    </button>
    ...
    ?>
    <button class="add-to-wishlist-btn-teen <?= $is_in_wishlist ? 'added' : '' ?>" 
            onclick="addToWishlistTeen(<?= $book['id'] ?>, this)">
    ...
    if ($total_pages > 1):
    ?>
    <div class="pagination-section-teen">
        <div class="pagination-teen">
            <?php if ($current_page > 1): ?>
            <a href="teen.php?page=1">« Първа</a>
            <?php endif; ?>
            
            <?php if ($current_page > 1): ?>
            <a href="teen.php?page=<?= $current_page - 1 ?>">‹</a>
            <?php endif; ?>
            ...
            <?php if ($current_page < $total_pages): ?>
            <a href="teen.php?page=<?= $total_pages ?>">Последна »</a>
            <?php endif; ?>
        </div>
        
        <div style="text-align: center; margin-top: 20px;">
            <?php if ($current_page > 1): ?>
            <a href="teen.php?page=<?= $current_page - 1 ?>" class="page-nav-btn-teen">
                ← Предишна страница
            </a>
            <?php endif; ?>
    ...
    <div class="empty-state-teen">
        <p>Все още няма налични книги за тийнеджъри.</p>
...
<script>
// ФУНКЦИЯ ЗА ДОБАВЯНЕ В КОШНИЦАТА - за тийнеджъри
function addToCartTeen(productId, button) {
    if (!button) return;
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            showSimpleMessage('✓ Книгата е добавена в кошницата!', 'success', 'teen');
        } else {
...
// Функция за прости съобщения с цвят за тийнеджъри
...
// ФУНКЦИЯ ЗА ДОБАВЯНЕ В ЛЮБИМИ (Тийнеджъри)
function addToWishlistTeen(bookId, button) {
    if (!button) return;
    
    // Проверка дали потребителят е влязъл
    <?php if (!isset($_SESSION['user_id'])): ?>
        showSimpleMessage('Моля, влезте в профила си за да добавите в любими!', 'info', 'teen');
        return;
    <?php endif; ?>
    ...
    if (isAdded) {
        ...
        removeFromWishlistTeen(bookId, button, originalIcon);
    } else {
        // Добавяне в любими
        button.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
        button.disabled = true;
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                showSimpleMessage('✓ Книгата е добавена в любими!', 'success', 'teen');
                ...
                showSimpleMessage('✗ ' + (data.message || 'Грешка при добавяне'), 'error', 'teen');
            }
        })
        .catch(error => {
            console.error('Грешка:', error);
            button.disabled = false;
            button.innerHTML = originalIcon;
            showSimpleMessage('✗ Възникна грешка', 'error', 'teen');
        });
    }
}

// ФУНКЦИЯ ЗА ПРЕМАХВАНЕ ОТ ЛЮБИМИ (Тийнеджъри)
function removeFromWishlistTeen(bookId, button, originalIcon) {
    fetch('add_to_wishlist.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            showSimpleMessage('✓ Книгата е премахната от любими', 'success', 'teen');
            ...
            button.innerHTML = originalIcon;
            showSimpleMessage('✗ ' + (data.message || 'Грешка при премахване'), 'error', 'teen');
        }
    })
    .catch(error => {
        console.error('Грешка:', error);
...
// ФУНКЦИЯ ЗА ПРОВЕРКА ДАЛИ КНИГА Е В ЛЮБИМИ (за Тийнеджъри)
function checkWishlistStatusTeen() {
    <?php if (isset($_SESSION['user_id'])): ?>
    const bookIds = [];
    document.querySelectorAll('.add-to-wishlist-btn-teen').forEach(btn => {
        const match = btn.getAttribute('onclick')?.match(/addToWishlistTeen\((\d+)/);
        if (match) {
        ...
        .then(data => {
            if (data.success) {
                ...
                data.wishlist_books.forEach(bookId => {
                    const btn = document.querySelector(`[onclick*="addToWishlistTeen(${bookId}"]`);
                    if (btn) {
